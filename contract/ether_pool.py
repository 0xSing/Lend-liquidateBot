...
class EtherPool(Contract):
    abi = Contract._load_abi(__name__, '../abis/Ether.abi')
    ...
    def liquidateBorrow(self, borrower, repayAmount, pTokenCollateral, profit, gas_price, nonce):
        tx_hash = self.contract.functions.liquidateBorrow(borrower, pTokenCollateral).transact({
                    'from': profit.address,
                    'value': repayAmount,
                    'gasPrice': gas_price,
                    'gas': 1000000,
